...
import exceptions.NotEnoughtMoneyException;
...
public class Shop {
    ...
    public static void main(String[] args) {
        ...
        У customerIvanov есть метод buy(main.Shop, main.Item): shop - магазин, в котором осуществляется покупка,
        ...
        */
        ...
        метод writeInformationAboutShopping(String fileName, main.Item item). Если на позицию fileName вместо
        названия файла передать директорию, то метод выбросит IOException, которое внутри метода writeInformationAboutShopping
        оборачивается в exceptions.MyIOException и пробрасывается дальше (это соответствует пункту дз оборачивание ошибки
        в новую)
        */
        ...
        */
        ...
        (например, при вызове метода lend(main.Bank bank, int credit) аккаунт создаётся автоматически
        ...
        связи этот процесс может прерваться, так как метод putMoneyToAccount(money,main.Account)
        выбрасывает exceptions.BadConnectionException.
        ...
        удаён из ассоциативного массива main.Bank.accounts
         */
        ...
        //Данный метод позволяет посмотреть состояние массива main.Bank.accounts
        Bank.printBank();

        /*метод divideMoney(double money, int parts) класс main.Bank служит чтобы
